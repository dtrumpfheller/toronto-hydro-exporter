...
func Export(consumptions *list.List, config helpers.Config) {
	...
	query := `from(bucket: "` + config.InfluxDB.Bucket + `")
		...
		|> filter(fn: (r) => r["meter"] == "` + config.TorontoHydro.Meter + `")
	...
	if consumptions.Len() > 0 {
		...
		for e := consumptions.Front(); e != nil; e = e.Next() {
			...
			log.Println("Inserting " + consumption.Time.Format("2006-01-02 3pm"))
			point := influxdb2.NewPointWithMeasurement("toronto_hydro").
				AddTag("meter", config.TorontoHydro.Meter).
